...
The unit tests (napalm/tests/unit/) are meant to cover as much code as
possible and should be executed without the service running. They are
designed to test the various pieces of the NAPALM tree to make sure
any new changes don't break existing functionality.

The functional tests (napalm/tests/integration/) are intended to
...
no easy way to set a breakpoint in the NAPALM code, and enter an
...
NAPALM uses `tox`_ for managing the virtual
...
(Insert more information about additional Tox environments here as they are created)

For more information on the standard Tox-based test infrastructure used by
NAPALM and how to do some common test/debugging procedures with Testr,
...
Tox packages and installs the NAPALM source tree in a given venv

#!/usr/bin/env bash
# Copyright (c) 2016 eBay, Inc., All Rights Reserved.
...
napalm_path=$(cd "$(dirname "$0")/.." && pwd)
base_test_path=napalm/tests/unit
test_path=$napalm_path/$base_test_path
...
for test_file in ${test_files[@]}; do
    ...
    expected_path=$(dirname $napalm_path/napalm/$relative_path)
